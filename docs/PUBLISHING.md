...
## Important: Built Files

**Never manually edit the built visualizer files** (`breakout-grid-visualizer.js` and `breakout-grid-visualizer-lite.js`).

These files are generated by the build process, which automatically injects the version from `package.json` using the `__VERSION__` placeholder defined in `vite.config.js`.

When you need to update these files:
1. Make changes to the source files in `src/visualizer/`
2. Run `npm run build` to regenerate the built files
3. Run `npm run check-version` to verify the version is correct
4. Commit both the source changes and the regenerated built files

This ensures version numbers stay in sync and the build process remains the single source of truth.
